...
本项目来自一个 **Xb 社区 × 灵心巧手** 的公益线上教学实践场景。

背景大致如下：

- Xb 社区开展灵巧手相关的公益网上教学项目
- 灵心巧手为该教学项目提供了硬件赞助支持
- 社区以接近成本价采购了 O6 实物灵巧手，供学员尽快进入真实设备控制阶段
- 本项目是在这一背景下完成的一个演示型上位机程序，目标是帮助后续学员更快把 O6 实物“控制起来、跑起来、改起来”
...
- **Xb 社区学员**：希望尽快从“会装环境”过渡到“能控制实物灵巧手”
- **机器人/具身方向初学者**：希望通过一个相对直接的 GUI 样例理解 O6 控制链路
- **二次开发者**：希望在现有 SDK 基础上快速改出自己的控制逻辑、交互界面或实验功能
- **教学组织者 / 实验助教**：希望有一个可讲解、可演示、可复用的起步项目
...
本仓库采用 **MIT License** 开源发布。

这意味着你可以在保留原始版权声明和许可证文本的前提下：
...
但本软件按 **"AS IS"** 提供，不附带任何明示或暗示担保。

### 15.1 与上游 SDK 的关系

本项目依赖 `linkerbot-python-sdk` 提供的 O6 控制能力。

根据本地参考的上游 SDK 许可证文件：

- `linkerbot-python-sdk-main/LICENSE:1` → `MIT License`
- `linkerbot-python-sdk-main/LICENSE:3` → `Copyright (c) 2026 linker-bot`

因此：

- 本仓库自身代码按 **MIT License** 发布
- 所依赖的第三方库和上游 SDK 仍分别遵循其各自许可证
- 如果你在后续分发中直接复制了上游 SDK 的源码或其重要片段，请同时保留对应版权声明与许可证文本
...
这些依赖**不因为本仓库采用 MIT 就自动变成 MIT**；它们仍然受各自原始许可证约束。
